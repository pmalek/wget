...
Downloader::Downloader(boost::asio::io_service& ioservice)
    : ioservice(ioservice), tcp_socket(ioservice), resolv(ioservice)
{}
...
void Downloader::go()
{
  for(const auto &uri : uris)
  {
    ...
    resolv.async_resolve(tcp::resolver::query{uri.host().to_string(),
                                              (uri.scheme().to_string() == "https" ? "443" : "80")},
    ...
    std::shared_ptr<beast::streambuf>)
{
  ...
  if((response->status == 301 || response->status == 302) && response->fields.exists("Location"))
  {
    ...
    resolv.async_resolve(tcp::resolver::query{u.host().to_string(),
                                              (u.scheme().to_string() == "https" ? "443" : "80")},
